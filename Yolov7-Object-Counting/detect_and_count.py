        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                   ...
                   area_coords = [(66,82), (774,80), (794,784), (70,789)]
                   Area_Coords=[(49,66),(801,59),(805,806),(59,810)]
...
# Iterate over each detected object (person)
    ...
    # Extract the coordinates of the detected person
                       ...
                       centroids.append(centroid)  # add to the list

      # Check if the coordinates of the detected person are within the boundaries of the area
                       if cv2.pointPolygonTest(np.array(area_coords), (x_center, y_center), False) >= 0 and conf>=0.50:
        # If the person is within the area of interest, mark the person in the image
                           counter+=1
                           label = f'{names[int(cls)]} {conf:.2f} ({x1},{y1}) ({x2},{y2}) '
                           area = (x2 - x1) * (y2 - y1) # calculate the area of the bounding box
                           label += f'{area:.2f} pixels' # add the area to the label
                           x_center, y_center = (x1 + x2) // 2, (y1 + y2) // 2 
                           plot_one_box((x1, y1, x2, y2), im0, label=label, color=colors[int(cls)], line_thickness=1)
                           cv2.putText(im0, f'{counter} people detected in area', (450, 250), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                           cv2.circle(im0,centroid,5,(0,255,0),-1)
                           pts = np.array(centroids, np.int32)
                           pts = pts.reshape((-1, 1, 2))
                           cv2.polylines(im0, [pts], False, (0, 0, 255), 2)
